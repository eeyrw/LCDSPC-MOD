...
int custchar[9] = {0, 0, 1, 2, 3, 4, 5, 6, 7};
...
extern "C" DLLEXPORT char *__stdcall function1(char *param1, char *param2)
{
    ...
    // Brute force work out the correct width
    width = atoi(param2);
    if (width >= 32)
        width = 32;
    else if (width >= 16)
...
extern "C" DLLEXPORT char *__stdcall function4(char *param1, char *param2)
{
    ...
    if (!strncmp(param1, "L", 1))
        sprintf(str, "%d", SPC_get_level(1, param2[0] - '0', sm));
    else if (!strncmp(param1, "R", 1))
        sprintf(str, "%d", SPC_get_level(2, param2[0] - '0', sm));
    else
        sprintf(str, "%d", SPC_get_level(0, param2[0] - '0', sm));
    return str;
}
...
extern "C" DLLEXPORT char *__stdcall function5(char *param1, char *param2)
{
    ...
    if (param2[1] = '#')
        sm = atoi(&param2[2]);
    if (!strncmp(param1, "L", 1))
        sprintf(str, "%d", SPC_get_level(1, param2[0] - '0', sm) / 328);
    else if (!strncmp(param1, "R", 1))
        sprintf(str, "%d", SPC_get_level(2, param2[0] - '0', sm) / 328);
    else
        sprintf(str, "%d", SPC_get_level(0, param2[0] - '0', sm) / 328);
    return str;
}
